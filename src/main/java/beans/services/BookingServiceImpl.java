...
public class BookingServiceImpl implements BookingService {
    ...
    @Override
    public double getTicketPrice(String eventName, String auditoriumName, LocalDateTime dateTime, List<Integer> seats, User user) {
        ...
        if (Objects.isNull(event)) {
            throw new IllegalStateException("There is no event with name: [" + eventName + "] in auditorium: [" + auditorium + "] on date: [" + dateTime + "]");
        }
        ...
        final List<Integer> vipSeats = auditoriumVipSeats.stream().filter(seats::contains).collect(Collectors.toList());
        final List<Integer> simpleSeats = seats.stream().filter(seat -> !vipSeats.contains(seat)).collect(Collectors.toList());
    ...
    public Ticket getTicketById(Long ticketId) {
        return bookingDAO.getTicketById(ticketId);
    }
